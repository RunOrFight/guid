...
type TDeleteTreeNodeFormProps = Pick<IModalContext, "closeModal"> & Pick<ITreeItem, "id">
...
const renderTrigger = ({openModal}: IModalContext) => <TrashIcon size={"14"} onClick={withStopPropagation(openModal)}/>

const DeleteTreeNode: FC<Pick<ITreeItem, "id">> = ({id}) => {
    return <Modal renderTrigger={renderTrigger}>
        {({closeModal}: IModalContext) => <DeleteTreeNodeForm closeModal={closeModal} id={id}/>}
    </Modal>
}
